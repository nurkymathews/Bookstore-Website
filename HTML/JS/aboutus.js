...
document.addEventListener("DOMContentLoaded", function(event)
{
  ...
  $(document).ready(function() {
    $(".team").mouseover(function() {
        $(".team-img").fadeIn("slow");
        $("#box1txt .h3").fadeIn("slow");
    });
});
             
  $("#learn_more").click((event)=>{
    event.preventDefault();
    $("#learn_more").hide("slow");
        $("#open_bookstore_txt").slideDown("slow");
        $("#showless").slideDown("slow");
})

  $("#showless").click(()=>{
    $("#learn_more").show("slow");
        $("#showless").slideUp("slow")
            $("#open_bookstore_txt").slideUp("slow");
  })


 $(document).ready(function() {
  var formSubmitted = false; // Initialize a var to track form submission

  // Show the form when user scrolls a certain distance
  $(window).scroll(function() {
      if (!formSubmitted && $(this).scrollTop() > 1000) {  
          $('.scrollform').fadeIn();
      } else {
          $('.scrollform').fadeOut();
      }
  ...
  $('#subscribe-form').submit(function(event) {
      ...
      $('.scrollform').hide(function() {
          // Show an alert after hiding the form
